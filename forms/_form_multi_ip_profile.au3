...
Global $hMultiIP_ProfileWindow  ; Handle della finestra Multi IP
Global $lbox_MultiIP_profile
Global $listIP_profile ; Lista globale per memorizzare gli IP del profilo in modifica


Func _form_multi_ip_profile()
	$w = 335 * $dScale
	$h = 260 * $dScale
    ...
    $hMultiIP_ProfileWindow = $settingsChild
	...
	Global $lbox_MultiIP_profile = GUICtrlCreateListView("IP|Subnet", 10 * $dScale, 10 * $dScale, $w - 20 * $dScale, $h - 70 * $dScale, BitOR($LVS_REPORT, $LVS_SINGLESEL, $LVS_SHOWSELALWAYS))
	;set dimensions of columns
	; Imposta le larghezze delle colonne (opzionale)
	_GUICtrlListView_SetColumnWidth($lbox_MultiIP_profile, 0, 150)  ; Colonna 0: 150px
	_GUICtrlListView_SetColumnWidth($lbox_MultiIP_profile, 1, 200)  ; Colonna 1: 200px
	; ================================================
    ; POPOLA LA LISTVIEW CON I DATI DI $listIP_profile
    ; ================================================
    
    ; Verifica che $listIP sia un array valido e contenga dati
    If IsArray($listIP_profile) And $listIP_profile[0][0] > 0 Then
...
EndFunc   ;==>_form_multi_ip_profile
